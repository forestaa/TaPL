...
instance FindVar Type 'True TypedBinding where
  findvar _ ctx x = V.findIndex isBound ctx
    where
      isBound (x', VariableTypeBind _)     = x == x'
      isBound (x', TypeAbbreviationBind _) = x == x'
      isBound _ = False
...
data Term a =
  ...
  | AscribeTerm (Record '["term" :> Term a, "as" :> Type a])
...
instance (Show (Named a), Show (Type a), Show (Term a)) => Show (Term a) where
  ...
  show (AscribeTerm t) = concat [show (t ^. #term), " as ", show (t ^. #as)]
...
instance FindVar Term 'True TypedBinding where
  findvar _ ctx x = V.findIndex isBound ctx
    where
      isBound (x', VariableTermBind _)     = x == x'
      isBound _ = False
...
instance (FindVar Term a b, FindVar Type a b, Binding Term b, Binding Type b) => NameLess Term a b where
  ...
  nameless (LetTerm t) = do
    body <- nameless $ t ^. #body
    expr <- local (V.cons (t ^. #name, binding (Proxy :: Proxy Term))) $ nameless (t ^. #in)
    return . LetTerm $ #name @= t ^. #name <: #body @= body <: #in @= expr <: nil
  nameless (AscribeTerm t) = do
    term <- nameless $ t ^. #term
    as   <- nameless $ t ^. #as
    return . AscribeTerm $ #term @= term <: #as @= as <: nil
instance IndexOperation Term where
  ...
  indexMap c (LetTerm t) = do
    body <- indexMap c $ t ^. #body
    expr <- indexMap (c+1) $ t ^. #in
    return . LetTerm $ t & #body .~ body & #in .~ expr
  indexMap c (AscribeTerm t) = do
    term <- indexMap c $ t ^. #term
    as   <- asksEff #ontype (\ontype -> ontype c $ t ^. #as)
    return . AscribeTerm $ t & #term .~ term & #as .~ as
...
eval1 (LetTerm t) | isVal (t ^. #body) = Just $ betaReduction (t ^. #body) (t ^. #in)
                  | otherwise = (\body -> LetTerm $ t & #body .~ body) <$> eval1 (t ^. #body)
eval1 (AscribeTerm t) | isVal (t ^. #term) = Just $ t ^. #term
                      | otherwise = (\term -> AscribeTerm $ t & #term .~ term) <$> eval1 (t ^. #term)
eval1 _ = Nothing
...
typing (LetTerm t) = do
  body <- typing (t ^. #body)
  indexShift (-1) <$> local (V.cons (t ^. #name, VariableTermBind body)) (typing (t ^. #in))
typing (AscribeTerm t) = do
  let as = t ^. #as
  whole <- restoreNameInTyping $ AscribeTerm t
  as' <- restoreNameInTyping as
  k <- mapLeftDef (KindingError as' whole) $ kinding as
  if k /= StarKind
    then throwError $ StarKindExpectedWithTerm k as' whole
    else do
      term <- typing $ t ^. #term
      mapLeftDef PromotingError (term `isSubTypeOf` as) >>=
        bool
          (failed2 term as)
          (return as)
  where
    failed2 term as = do
      ...
      as' <- restoreNameInTyping as
      whole <- restoreNameInTyping $ AscribeTerm t
      throwError $ SubTypeExpected t' term' as' whole
...
leaveTypingAssertion :: TypingContext -> NamedTerm -> NamedType -> Either Errors Bool
leaveTypingAssertion ctx t ty = leaveEff . (`runReaderDef` ctx) . runEitherDef $ do
  t' <- mapLeftDef (NameLessError . UnNameError) $ unName t
  ans <- mapLeftDef (NameLessError . UnNameError) $ unName ty
  got <- mapLeftDef TypingError $ typing t'
  castEff $ got `isEquivalentTo` ans

leaveSubTypingAssertion :: TypingContext -> NamedTerm -> NamedType -> Either Errors Bool
leaveSubTypingAssertion ctx t ty = leaveEff . (`runReaderDef` ctx) . runEitherDef $ do
  t' <- mapLeftDef (NameLessError . UnNameError) $ unName t
  ans <- mapLeftDef (NameLessError . UnNameError) $ unName ty
  got <- mapLeftDef TypingError $ typing t'
  mapLeftDef (TypingError . PromotingError) $ got `isSubTypeOf` ans